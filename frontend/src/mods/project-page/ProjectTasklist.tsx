...
import {
	CheckSquareIcon, ChevronLeftIcon, ChevronRightIcon, ChevronsRightIcon, Clock8Icon, EditIcon, InfoIcon, ListTodoIcon, MoreHorizontalIcon, SquareIcon, TrashIcon, XIcon
} from 'lucide-react'
	...
	return (
		<div className='pt-3 flex flex-col items-stretch'>
			...
			{data.map((task) => (
				...
				>
					<div
						className='p-1.5 mt-0.5 mr-0.5 hover:dark:bg-black/20 hover:active:dark:bg-black/40 hover:bg-neutral-200 active:hover:bg-neutral-300 rounded-md text-neutral-500 hover:!text-primary-500 cursor-pointer transition-colors group'
					...
					>
						...
						{ task.status === 'IN_PROGRESS' && <Clock8Icon className='size-[18px]' /> }
						{ task.status === 'DEFERRED' && <ChevronsRightIcon className='size-[18px] text-purple-500 group-hover:dark:text-purple-400 group-hover:text-purple-600' /> }
					...
					<div className='pt-[7px]'>{task.title}</div>

					<div className='absolute opacity-0 group-hover/task:opacity-50 hover:opacity-100 group-hover/task:hover:opacity-100 right-0.5 top-0.5 p-1.5 cursor-pointer group/optionbtn transition-opacity hover:dark:bg-black/20 hover:bg-neutral-200 rounded-md active:dark:bg-black/40 active:bg-neutral-300' onClick={() => (showContextMenuTaskID === task.id ? setShowContextMenuTaskID(null) : setShowContextMenuTaskID(task.id))}>
